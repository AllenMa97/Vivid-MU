...
| 配置项 | 默认值 | 含义 |
|---|---|---|
| `capture.retention_hours` | 72 小时 | 原始片段滚动保留窗口，过期自动删除 |
...
| 分辨率 | 每小时占用 | 72 小时滚动缓存 |
|---|---|---|
| 640×480（推荐） | ~1–3 GB | ~24–90 GB |
| 1280×720 | ~2–5 GB | ~48–150 GB——P20 扛不住 |

P20 总共只有 64–128 GB 存储，所以分辨率调高时请顺手把 `capture.retention_hours` 调小（比如 24），或在 IP Webcam 里降低分辨率。高光本身很小（只是几秒到几十秒的短片），不用担心。随时可以在 Termux 里跑 `python main.py status` 查看磁盘水位、待处理片段数和最近文件。
...
| `capture.retention_hours` | 72 | 原始录像滚动保留窗口 |
